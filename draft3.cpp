...
public:
    ...
    std::vector<std::string> combosnew;
    ...
    void comb(int N, int K)
    {
        ...
        do {
            for (int i = 0; i < N; ++i) // [0..N-1] integers
            {
                ...
                combos.push_back(bitmask[i]);combos.push_back(i);//GET RID OF REPEATS THATS WHAT IT DOES
            }
            //std::cout << std::endl;
        } while (std::prev_permutation(bitmask.begin(), bitmask.end()));
        int x = combos.size();
        for(int i =1;i<x;i++){
            std::string a = std::to_string(combos[2*i]);
            std::string b = std::to_string(combos[2*i + 1]);//index is wrong
            std::string ab = a+b;
            combosnew.insert(combosnew.end(),ab);
        }
    }
...
int main()
{
    ...
    int  yx = fileobj.combos.size();
     for(int i=0;i<yx;i++){
        std::cout<< "new" <<fileobj.combosnew[i]<<" ";
     }
     auto end = fileobj.combosnew.end();
         for (auto it = fileobj.combosnew.begin(); it != end; ++it) {
             end = std::remove(it + 1, end, *it);
         }

         fileobj.combosnew.erase(end, fileobj.combosnew.end());
         for(int i=0;i<yx;i++){
            std::cout<< fileobj.combosnew[i]<<","; //WHY DOES IT HAVE ALL THE REST??? CONFUSED
         }
